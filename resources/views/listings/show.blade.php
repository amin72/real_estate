...
<section class="bg-secondary opacity-80 text-white px-8 sm:px-40 xl:px-96 py-16 text-center">
  ...
  <h2 class="text-3xl mt-4">{{ $listing->zone->name }}</h2>
  <p class="text-2xl mt-4">{{ $listing->address }}</p>
  <p class="text-2xl mt-4">شماره آگهی: {{ $listing->id }}</p>
</section>
...
<main class="px-8 sm:px-40 xl:px-96 my-10">
  ...
  <div class="text-secondary my-10 flex flex-col md:flex-row flex-wrap justify-around">
    ...
    @if ($listing->category->slug === 'اجاره')
      <!-- price monthly -->
      <div class="w-full md:w-1/2 mx-auto px-4">
        <div class="border-b flex flex-row justify-between py-2 px-4">
          <div class="flex flex-row items-center">
            ...
            <span class="mr-1">اجاره ماهانه:</span>
          </div>
          <div class="">
            <span class="change_price ml-1 tracking-widest">{{ $listing->price_monthly }}</span>
            تومان
          </div>
        </div>
      </div>
    ...
    <div class="w-full md:w-1/2 mx-auto px-4">
      <div class="border-b flex flex-row justify-between py-2 px-4">
        <div class="flex flex-row items-center">
          ...
          <span class="mr-2">آسانسور:</span>
        </div>
        <div class="">
        <span>{{ $listing->has_elevator ? 'دارد' : 'ندارد' }}</span>
        </div>
      </div>
    </div>
  ...
  <div class="w-full mx-auto px-4">
    ...
    <div class="">
      <div class="flex items-center font-semibold justify-center">
        <svg class="h-4 w-4 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          <path d="M5 5a5 5 0 0 1 10 0v2A5 5 0 0 1 5 7V5zM0 16.68A19.9 19.9 0 0 1 10 14c3.64 0 7.06.97 10 2.68V20H0v-3.32z"/>
        </svg>
        <span class="mr-2 text-lg">راه های تماس</span>
      </div>
      <div class="flex flex-col sm:flex-row flex-wrap justify-between mt-4 text-lg">
        <!-- whatsapp -->
        <div class="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 2xl:w-1/5 flex flex-row items-center font-semibold justify-around my-3">
          <svg class="h-8 w-8 w-1/3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
          ...
          <a class="w-2/3" href="https://wa.me/{{ $manager->whatsapp }}">{{ $manager->whatsapp }}</a>
        </div>
        ...
        <div class="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 2xl:w-1/5 flex flex-row items-center font-semibold justify-around my-3">
          ...
          <span class="w-2/3">{{ $manager->mobile }}</span>
        </div>

        <!-- phone -->
        <div class="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 2xl:w-1/5 flex flex-row items-center font-semibold justify-around my-3">
          <svg class="h-8 w-8 w-1/3" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 392.788 392.788" style="enable-background:new 0 0 392.788 392.788;" xml:space="preserve">
          ...
          <span class="w-2/3">{{ $manager->phone }}</span>
        </div>
        ...
        <div class="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 2xl:w-1/5 flex flex-row items-center font-semibold justify-around my-3">
          ...
          <a class="w-2/3" href="https://www.instagram.com/{{ $manager->instagram }}/">{{ $manager->instagram }}</a>
        </div>
        ...
        <div class="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 2xl:w-1/5 flex flex-row items-center font-semibold justify-around my-3">
          ...
          <a class="w-2/3" href="https://www.t.me/{{ $manager->telegram }}/">{{ $manager->telegram }}</a>
        </div>
